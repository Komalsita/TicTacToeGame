...
class TicTacToe {
    ...
    // NEW METHOD: Checks if the board is full for a Draw
    static boolean checkDraw() {
        for (int i = 0; i <= 2; i++) {
            for (int j = 0; j <= 2; j++) {
                if (board[i][j] == ' ') {
                    return false; // Found an empty spot, game continues
                }
            }
        }
...
abstract class Player{
    ...
    boolean isValidMove(int row, int col) {
        ...
        System.out.println("Invalid Move, try again."); // Feedback for user
        return false;
    }
}
class HumanPlayer extends Player {
    ...
    void makeMove() {
        ...
        int col;
        do {
            System.out.println("Enter the Row and col (0-2):");
            row = scan.nextInt();
            col = scan.nextInt();
